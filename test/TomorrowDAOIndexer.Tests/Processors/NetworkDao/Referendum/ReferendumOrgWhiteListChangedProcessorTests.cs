...
public class ReferendumOrgWhiteListChangedProcessorTests : TomorrowDAOIndexerTestBase
{
    [Fact]
    public async Task ProposalCreated_Test()
    {
        var logEvent = NetworkDaoOrgWhiteListChanged();
        await ReferendumOrgWhiteListChangedProcessor.ProcessAsync(GenerateLogEventContext(logEvent));
