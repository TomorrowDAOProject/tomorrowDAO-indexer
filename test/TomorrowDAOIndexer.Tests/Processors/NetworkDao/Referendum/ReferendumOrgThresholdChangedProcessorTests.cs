...
public class ReferendumOrgThresholdChangedProcessorTests : TomorrowDAOIndexerTestBase
{
    [Fact]
    public async Task ProposalCreated_Test()
    {
        var logEvent = NetworkDaoOrgThresholdChanged();
        await ReferendumOrgThresholdChangedProcessor.ProcessAsync(GenerateLogEventContext(logEvent));
